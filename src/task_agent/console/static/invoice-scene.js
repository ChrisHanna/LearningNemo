(() => {
  ...
  const definitions = {
    ...
    saw: { title: 'SAW workspace', icon: 'server', subtitle: 'Azure-hosted runtime', why: 'The managed envelope around a private VM dedicated to the website agent. The workspace hosts isolated agent runs with per-run time limits and restricted networking; the website itself runs outside it.', allowed: ['Host one active invoice run', 'Retain stopped sandboxes'], denied: ['Public inbound access', 'Direct SQL and metadata network access'] },
    ...
    model: { title: 'Model + Guardrails', icon: 'sparkles', subtitle: 'Mediated inference', why: 'NeMo Guardrails checks untrusted input and tool results before inference, and checks model output and proposed tool calls before they reach the agent. The provider credential stays in the trusted gateway, not the sandbox.', allowed: ['Check untrusted narrative', 'Forward admitted inference'], denied: ['Grant database privileges', 'Replace human approval'] },

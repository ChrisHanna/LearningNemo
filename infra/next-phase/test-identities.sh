...
parameter_file="$phase_dir/environments/dev.platform.parameters.json"
...
bash -n "$phase_dir/deploy-identities.sh"
bash -n "$phase_dir/remove-identities.sh"
bash -n "$phase_dir/test-identities.sh"
PYTHONPYCACHEPREFIX="$pycache_dir" python3 -m py_compile \
  "$phase_dir/platform_contract.py" \
  "$phase_dir/platform_parameters.py" \
  "$phase_dir/preflight_identities.py" \
  "$phase_dir/record_identities.py" \
  "$phase_dir/summarize_what_if.py" \
  "$phase_dir/validate-identities.py" \
  "$phase_dir/verify_identities.py"
python3 "$phase_dir/platform_parameters.py" validate "$parameter_file"
python3 "$phase_dir/preflight_identities.py" --offline --parameters "$parameter_file"
az bicep build --file "$phase_dir/identities.bicep" --stdout > "$compiled_template"
python3 "$phase_dir/validate-identities.py" "$compiled_template"
"$test_python" -m pytest -q tests/test_next_phase_identity_infra.py

if [[ "$azure" == true ]]; then
  bash "$phase_dir/register-providers.sh" --phase identities
  bash "$phase_dir/deploy-identities.sh" --what-if --parameters "$parameter_file"
else
  echo "PASS local identity tests complete; Azure state was not queried or changed."
